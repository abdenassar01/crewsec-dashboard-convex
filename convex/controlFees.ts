...
const controlFeeStatus = v.union(v.literal("AWAITING"), v.literal("PAID"), v.literal("CANCELED"), v.literal("CONFLICT"));
  ...
  handler: async (ctx, args) => {
    ...
    if (args.violationId) {
        ...
        return (await feesQuery.collect())
            ...
            .slice(args.paginationOpts.numItems * (args.paginationOpts.cursor ? parseInt(args.paginationOpts.cursor) : 0), args.paginationOpts.numItems);
    }
...
export const deleteFee = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (fee && fee.galleryStorageIds) {
            await Promise.all(fee.galleryStorageIds.map(id => ctx.storage.delete(id)));
        }
