...
export function VehicleListClient({ preloadedVehicles }: { preloadedVehicles: Preloaded<typeof api.vehicles.list> }) {
  const [isDialogOpen, setIsDialogOpen] = React.useState(false);
  const [editingVehicle, setEditingVehicle] = React.useState<Doc<"vehicles"> | null>(null);

  const { page, isDone, continueCursor } = usePreloadedQuery(preloadedVehicles);
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        columns={columns}
        data={page}
        pageCount={isDone ? (page.length / 10) : (page.length / 10) + 1}
        pageIndex={Math.floor(page.length / 10) - 1}
        pageSize={10}
